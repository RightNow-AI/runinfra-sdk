...
TypeScript request interfaces are closed around typed fields. Use `extraBody` in request options for deliberate JSON body extensions, such as an unsupported-parameter canary. `extraBody` cannot override typed request fields and is validated before the request is sent.
